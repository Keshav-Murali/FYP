...
double RngStream::RandU01 ()
{
 ...
 return (double) LXM->generateNumber() / std::numeric_limits<uint64_t>::max();
}
...
RngStream::RngStream (uint32_t seedNumber, uint64_t stream, uint64_t substream)
{
  ...
  */
  ...
  std::vector<uint64_t> v1 {stream, substream};
  l = new LCGenerator(v1);
  x = new xoroshiroGenerator(v1);
  LXM = new combinationGenerator<uint64_t>(l, x, &lea64);
}
